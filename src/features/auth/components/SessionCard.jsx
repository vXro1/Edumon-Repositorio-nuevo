    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
          <span style={{ fontSize: 13.5, fontWeight: 600, color: "var(--color-text, #111)", textTransform: "capitalize" }}>
            {deviceType}
          </span>
          {isCurrent && (
            <span style={{
              ...
              padding: "2px 8px", borderRadius: 9999, letterSpacing: "0.04em",
            }}>
              ACTUAL
            </span>
          )}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "4px 12px" }}>
          ...
          {(ciudad || pais) && (
            <span style={{ fontSize: 12, color: "var(--color-text-muted, #6b7280)", display: "flex", alignItems: "center", gap: 4 }}>
              <MapPin size={11} />{[ciudad, pais].filter(Boolean).join(", ")}
            </span>
          )}
        </div>
      </div>
      ...
      <div style={{ textAlign: "right", flexShrink: 0 }}>
        {ultimaActividad && (
          <div style={{ fontSize: 12, color: "var(--color-text-muted, #6b7280)", display: "flex", alignItems: "center", gap: 4, justifyContent: "flex-end", marginBottom: 2 }}>
            <Clock size={11} />{formatDate(ultimaActividad)}
          </div>
        )}
        {fechaInicio && (
